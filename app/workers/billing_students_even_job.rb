class BillingStudentsEvenJob
  ...
  def perform(*_args)
    ...
    if weekend?(today)
      ...
      send_billing_emails_for_even_students
    end
  end
  ...
  def reschedule_job(today, days_to_add)
    ...
    BillingStudentsevenJob.perform_at(next_week_day.beginning_of_day)
  end

  def send_billing_emails_for_even_students
    set_student.select { |student| student.id.even? }.each do |student|
